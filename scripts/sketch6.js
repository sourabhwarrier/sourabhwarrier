...
const NUM_LAYERS = 6; /* could do 8 but this laptop sucks and so do most phones :(*/
...
function init() {
  ...
  camera =
    ...
    /*console.log(
      `Initial Camera Position: x=${camera.position.x}, y=${camera.position.y}, z=${camera.position.z}`
...
document.body.appendChild(renderer.domElement);
  ...
  controls.minDistance = 40; /*current */
  ...
  // Log camera position whenever OrbitControls changes it
  controls.addEventListener("change", () => {
    /*console.log(
      `Camera Position: x=${camera.position.x.toFixed(3)}, y=${camera.position.y.toFixed(3)}, z=${camera.position.z.toFixed(3)}`
    );*/

      // Redirect when camera reaches the required position
    if (
      (camera.position.x >= -0.01 && camera.position.x <=0.01) &&
      camera.position.y <= -4122.952 &&
      (camera.position.z >= -0.01 && camera.position.z <=0.01)
    ) {
